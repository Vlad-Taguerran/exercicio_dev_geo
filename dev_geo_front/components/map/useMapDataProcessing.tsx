...
const createGeoJsonFeatureCollection = (data: ICensusPoint[]): FeatureCollection<Point, GeoJsonProperties> => {
    return {
        ...
        features: data.map((point) => ({
            ...
            properties: Object.keys(point)
                ...
                .reduce((obj: GeoJsonProperties, key) => { // Tipagem explícita do acumulador
...
export function useMapDataProcessing(
    ...
    const [pinCount, setPinCount] = useState<number>(0); // Manter se usado em outro lugar
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [pinsInsidePolygon, setPinsInsidePolygon] = useState<ICensusPoint[]>([]); // Manter se usado
    ...
    const convertToICensusPoint = useCallback((properties: GeoJsonProperties, coordinates: [number, number]): ICensusPointDto => {
        // Removido console.log para produção, pode adicionar de volta para debug
    ...
    const setupSourceAndLayer = useCallback((currentMap: Map, data: ICensusPoint[]) => {
        ...
        const existingSource = currentMap.getSource(mapBoxConfig.SOURCE_ID) as GeoJSONSource | undefined;
            ...
            try {
                ...
                currentMap.addSource(mapBoxConfig.SOURCE_ID, {
                    ...
                    clusterProperties: {
                        ...
                        "sum_particular": ["+", ["coalesce", ["get", "censo_2022_domicilio_particular_poi_counts"], 0]],
                        "sum_agro": ["+", ["coalesce", ["get", "censo_2022_estabelecimento_agro_poi_counts"], 0]],
                        "sum_construcao": ["+", ["coalesce", ["get", "censo_2022_estabelecimento_construcao_poi_counts"], 0]]
             ...
             console.warn(`⚠️ Fonte '${mapBoxConfig.SOURCE_ID}' já existia. Atualizando dados (inesperado nesta abordagem).`);
             existingSource.setData(geoJsonData);
             sourceExists.current = true; // Garante que o ref está true
        } else {
            // Ref diz que existe, mas getSource não encontrou? Estranho.
             console.warn(`⚠️ Inconsistência: Ref sourceExists=true, mas getSource('${mapBoxConfig.SOURCE_ID}') falhou.`);
        ...
        if (sourceExists.current && isIconLoaded) {
            ...
            if (!clusterCircleLayerExists.current && !currentMap.getLayer( "clusters-circle")) {
                ...
                try {
                    ...
                    currentMap.addLayer({
                        ...
                        paint: {
                            "circle-color": ["step", ["get", "point_count"], "#51bbd6", 100, "#f1f075", 750, "#f1465d"], // Exemplo de cores/passos
                            "circle-radius": ["step", ["get", "point_count"], 20, 100, 30, 750, 40], // Exemplo de raios/passos
                            // Ajuste paint properties conforme mapBoxConfig ou design
                        },
                    });
                    clusterCircleLayerExists.current = true;
                    console.log(`   ✅ Camada '${layerId}' adicionada.`);
                } catch (error) { console.error(`   ❌ Erro ao adicionar camada '${layerId}':`, error); }
            }

            // 2. Camada de TEXTO para Clusters
            if (!clusterTextLayerExists.current && !currentMap.getLayer("cluster-count-text")) {
                ...
                try {
                    ...
                    currentMap.addLayer({
                        ...
                        filter: ["has", "point_count"],
                        layout: {
                            // Exibir contagem de pontos ou uma das propriedades agregadas
                            "text-field": ["get", "point_count_abbreviated"], // Ou ["get", "sum_particular"], etc.
                            "text-font": ["Open Sans Bold", "Arial Unicode MS Bold"], // Ajustar fontes
                            "text-size": 10 // Ajustar tamanho
                        },
                        paint: {
                            "text-color": "#ffffff" // Ajustar cor
                        }
                    });
            ...
            if (!unclusteredLayerExists.current && !currentMap.getLayer(mapBoxConfig.LAYER_ID)) { // Usa LAYER_ID do config
                ...
                try {
                    console.log(`   -> Adicionando camada '${layerId}' (pontos individuais)...`);
                    currentMap.addLayer({
                        id: layerId,
                        type: "symbol",
                        source: mapBoxConfig.SOURCE_ID,
             ...
             if (!sourceExists.current) console.log("ℹ️ Fonte não existe ou falhou ao adicionar, pulando adição de camadas.");
             if (!isIconLoaded) console.log("ℹ️ Ícone não carregado, pulando adição de camadas.");
        }
    ...
    useEffect(() => {
        ...
        console.log("🔄 Detectada mudança em dados/mapa. Resetando estado de processamento.");
        setProcessedData([]);
        setBatchIndex(0);
        ...
        const intervalId = setInterval(() => {
            ...
            setBatchIndex(currentIndex => {
                ...
                if (start >= censusData.length) {
                    ...
                    if (!isProcessingComplete.current) { // Evita logs/sets repetidos
                ...
                if (newBatch.length > 0) {
                    ...
                    // --- Lote Vazio (Fim inesperado?) ---
                     if (!isProcessingComplete.current) {
                        console.warn("⚠️ Lote vazio encontrado antes do fim dos dados. Considerando concluído.");
                        isProcessingComplete.current = true;
                        setAllDataProcessed(true); // <-- Sinaliza para o próximo efeito
                        clearInterval(intervalId); // Limpa o intervalo AQUI
                     }
    ...
    useEffect(() => {
        ...
        if (allDataProcessed && mapRef.current && isMapLoaded && isIconLoaded && processedData.length > 0) {

            // Verifica se a fonte já foi adicionada (para evitar re-execução desnecessária)
            if (sourceExists.current) {
                console.log("ℹ️ Fonte/camadas já configuradas anteriormente (pós processamento).");
                return;
            }

            console.log(`🚀 Configurando fonte e camadas com TODOS os ${processedData.length} pontos...`);
            try {
                ...
                if (currentMap.isStyleLoaded()) {
                    ...
                    console.warn("⚠️ Estilo do mapa não carregado ao tentar configurar fonte/camadas finais. Adicionando listener 'styledata'...");
                    const onStyleData = () => {
                        console.log("   -> Evento 'styledata' recebido.");
                        // Re-verifica todas as condições antes de tentar configurar
                         if(allDataProcessed && mapRef.current && isIconLoaded && processedData.length > 0 && !sourceExists.current) {
                              console.log("      -> Tentando configurar após 'styledata'...");
                              setupSourceAndLayer(mapRef.current, processedData);
                         } else {
                             console.log("      -> Condições não atendidas ou já configurado após 'styledata'.");
                         }
                ...
                console.error("❌ Erro durante o setup final (pós processamento) de SourceAndLayer:", error);
             ...
             console.warn("⚠️ Processamento concluído, mas não há dados processados para exibir.");
        }
    ...
    }, [allDataProcessed, processedData, mapRef, isMapLoaded, isIconLoaded, setupSourceAndLayer]);
        ...
        return () => {
            ...
            if (map) {
                 try {
                    ...
                    if (unclusteredLayerId && map.getLayer(unclusteredLayerId)) map.removeLayer(unclusteredLayerId);
                    if (map.getSource(mapBoxConfig.SOURCE_ID)) map.removeSource(mapBoxConfig.SOURCE_ID);
                    console.log("   ✅ Camadas e fonte removidas.");
                 } catch (e) {
                     console.warn("   ⚠️ Warn: Erro durante a limpeza de camadas/fonte no desmontar (pode ser normal se o mapa já foi destruído):", e);
                 }
            } else {
                console.log("   ℹ️ Ref do mapa não disponível no desmontar, limpeza de camadas/fonte pulada.");
            }
        };
        // A função de limpeza SÓ depende da existência do componente.
        // O array de dependências vazio [] garante que ela SÓ rode no desmontar.
        // Se precisar que rode se mapRef mudar (improvável), adicione [mapRef].
